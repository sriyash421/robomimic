...
class MeanFlowPolicyConfig(BaseConfig):
    ALGO_NAME = "mean_flow_policy"

    def train_config(self):
        """
        Setting up training parameters for Mean Flow Policy.

        - don't need "next_obs" from hdf5 - so save on storage and compute by disabling it
        - set compatible data loading parameters
        """
        super(MeanFlowPolicyConfig, self).train_config()
